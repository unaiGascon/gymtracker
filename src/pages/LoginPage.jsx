...
//
// Permite acceder con email+contraseña o con Google OAuth.
// Llama a onLogin cuando la sesión queda activa (App.jsx
// lo detecta vía onAuthStateChange y cambia la vista).

import { useState } from 'react'
import { supabase } from '../lib/supabase'

// Props:
//   onGoToRegister — navega a RegisterPage
export default function LoginPage({ onGoToRegister }) {
  ...
  async function handleSubmit(e) {
    ...
    // Si no hay error, onAuthStateChange en App.jsx detecta la sesión y cambia la vista
  }

  // Login con Google — redirige a Google y vuelve a la app automáticamente
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <p className="text-sm text-gray-400 mb-8 text-center">Accede a tu cuenta</p>
        ...
        <p className="text-center text-sm text-gray-500 mt-6">
          ¿No tienes cuenta?{' '}
          <button onClick={onGoToRegister} className="font-medium text-gray-900 underline">
            Regístrate
          </button>
        </p>
      </div>
